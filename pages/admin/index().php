...
if (!$stats) {
    ...
    $stats['visitors_per_month'] = DB::select('select YEAR(`day`) as "unique_visitors.year",MONTH(`day`) as "unique_visitors.month",count(id) as "unique_visitors.visitors" from `unique_visitors` where `day` >= NOW() - INTERVAL 30 MONTH group by YEAR(`day`),MONTH(`day`) order by YEAR(`day`) desc,MONTH(`day`) desc');
    ...
    $stats['subscribers_per_month'] = DB::select('select YEAR(`day`) as "unique_other_views.year",MONTH(`day`) as "unique_other_views.month",count(id) as "unique_other_views.visitors" from `unique_other_views` where `type` = \'feed\' and `day` >= NOW() - INTERVAL 30 MONTH group by YEAR(`day`),MONTH(`day`) order by YEAR(`day`) desc,MONTH(`day`) desc');
    ...
    $stats['homepage_visitors_per_month'] = DB::select('select YEAR(`day`) as "unique_other_views.year",MONTH(`day`) as "unique_other_views.month",count(id) as "unique_other_views.visitors" from `unique_other_views` where `type` = \'home\' and `day` >= NOW() - INTERVAL 30 MONTH group by YEAR(`day`),MONTH(`day`) order by YEAR(`day`) desc,MONTH(`day`) desc');

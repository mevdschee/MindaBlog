...
if (!empty($_POST)) {
	...
	if (!isset($errors)) {
		$rowsAffected = DB::update('UPDATE posts SET published = ?, slug = ?, title = ?, content = ? WHERE id = ?', $_POST['posts']['published'], $_POST['posts']['slug'], $_POST['posts']['title'], $_POST['posts']['content'], $id);

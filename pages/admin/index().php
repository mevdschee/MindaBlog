...
if (!$stats) {
    ...
    $stats['subscribers_per_month'] = DB::select('select YEAR(`day`) as "unique_other_views.year",MONTH(`day`) as "unique_other_views.month",count(id) as "unique_other_views.visitors" from `unique_other_views` where `type` = \'feed\' and `day` >= NOW() - INTERVAL 1 YEAR group by YEAR(`day`),MONTH(`day`) order by YEAR(`day`) desc,MONTH(`day`) desc');
    ...
    $stats['homepage_visitors_per_month'] = DB::select('select YEAR(`day`) as "unique_other_views.year",MONTH(`day`) as "unique_other_views.month",count(id) as "unique_other_views.visitors" from `unique_other_views` where `type` = \'home\' and `day` >= NOW() - INTERVAL 1 YEAR group by YEAR(`day`),MONTH(`day`) order by YEAR(`day`) desc,MONTH(`day`) desc');

...
if (!$stats) {
    ...
    $stats['homepage_visits_per_day'] = DB::select('select `day`,count(id) as "unique_other_views.visitors" from `unique_other_views` where `type` = \'home\' and `day` >= NOW() - INTERVAL 30 DAY group by `day` order by `day` desc');
    $stats['homepage_visits_per_month'] = DB::select('select YEAR(`day`) as "unique_other_views.year",MONTH(`day`) as "unique_other_views.month",count(id) as "unique_other_views.visitors" from `unique_other_views` where `type` = \'home\' and `day` >= NOW() - INTERVAL 1 YEAR group by YEAR(`day`),MONTH(`day`) order by YEAR(`day`),MONTH(`day`) desc');
...
$values = array_combine(
    array_map(function($v){return $v['unique_other_views']['day'];},$stats['homepage_visits_per_day']),
    array_map(function($v){return $v['unique_other_views']['visitors'];},$stats['homepage_visits_per_day'])
);
while (count($values)<30) array_push($values,0);
Buffer::set('daily_homepage_visits_graph',Graph::verticalBar($values,300,'Unique homepage visits per day'));
$values = array_combine(
    array_map(function($v){return sprintf('%04d-%02d',$v['unique_other_views']['year'],$v['unique_other_views']['month']);},$stats['homepage_visits_per_month']),
    array_map(function($v){return $v['unique_other_views']['visitors'];},$stats['homepage_visits_per_month'])
);
while (count($values)<12) array_push($values,'');
Buffer::set('monthly_homepage_visits_graph',Graph::verticalBar($values,300,'Unique homepage visits per month'));

<?php
$data = DB::select('select posts.id,posts.slug,posts.title,posts.word_count,posts.tags,posts.published,users.id,users.username,unique_views.requests from posts,users,unique_views where posts.user_id = users.id and unique_views.post_id = posts.id order by published is not null,published desc');

...
if (!isset($_SESSION['user'])) {
    DB::insert('insert into `unique_views` (`post_id`,`ip`,`day`,`referrer_id`,`user_agent_id`,`requests`) values (?,?,DATE(NOW()),?,?,1) ON DUPLICATE KEY UPDATE `requests`=`requests`+1;', $data['posts']['id'], $_SERVER['REMOTE_ADDR'], $_SESSION['referrer_id'], $_SESSION['user_agent_id']);
}
...
$url = rtrim(Router::getBaseUrl(), '/') . Router::getRequest();
$status = "$title... see: $url";

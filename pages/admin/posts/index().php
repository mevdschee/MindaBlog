<?php
$data = DB::select('select posts.id,posts.slug,posts.title,posts.word_count,posts.tags,posts.published,users.id,users.name from posts,users where posts.user_id = users.id order by published is not null,modified desc,published desc');
